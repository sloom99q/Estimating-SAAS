...
export const estimateKitchenHandler: JobHandler = async (job: JobRecord) => {
  ...
  // Use the most recently READY document for the project.
  ...
  const kitchens = await locateKitchens(
    job.organizationId,
    payload.projectId,
    document.id,
    sourceBytes,
  ...
  if (kitchens.length === 0) {
    return {
      ok: true,
      message:
        'No kitchen room found in extraction OR kitchen label could not be located on the source sheet. Confirm a ROOM with name containing KITCHEN exists.',
      estimates: [],
    }
  }
